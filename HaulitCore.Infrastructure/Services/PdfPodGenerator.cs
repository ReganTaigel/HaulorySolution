...
public class PdfPodGenerator : IPdfPodGenerator
{
    public byte[] GeneratePodPdf(PodReportDto dto)
    {
        ...
        var deliveredLocal = ToNzTime(dto.DeliveredAtUtc);
        canvas.DrawText($"Delivered: {deliveredLocal:yyyy-MM-dd HH:mm}", xLeft, y, SKTextAlign.Left, fontNormal, paintNormal);
        y += 14;
    ...
    $"[PdfPodGenerator] DamageNotes='{dto.DamageNotes}', WaitTimeMinutes={dto.WaitTimeMinutes}");
        ...
        var generatedLocal = ToNzTime(DateTime.UtcNow);
        // Footer
        canvas.DrawText(
            $"Generated by HaulitCore • {generatedLocal:yyyy-MM-dd HH:mm}",
            margin,
            height - 30,
            SKTextAlign.Left,
            fontSmall,
