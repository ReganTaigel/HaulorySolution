...
public class PdfInvoiceGenerator : IPdfInvoiceGenerator
{
    public byte[] GenerateInvoicePdf(InvoiceReportDto dto, byte[] signaturePngBytes)
    {
        ...
        canvas.DrawText($"Invoice date: {DateTime.Now:yyyy-MM-dd}", xLeft, y, SKTextAlign.Left, fontNormal, paintNormal);
        y += 14;
        ...
        if (dto.WaitTimeChargeEnabled && dto.WaitTimeChargeAmount > 0)
        {
            DrawRight(
                "Wait time:",
                dto.WaitTimeChargeAmount.ToString("C", culture));
            y += line;
        }

        if (dto.HandUnloadChargeEnabled && dto.HandUnloadChargeAmount > 0)
        {
            DrawRight(
                "Hand unload:",
                dto.HandUnloadChargeAmount.ToString("C", culture));
            y += line;
        }
        ...
        // Footer
        canvas.DrawText(
            $"Generated by HaulitCore • {DateTime.Now:yyyy-MM-dd HH:mm}",
            margin,
            height - 30,
            SKTextAlign.Left,
            fontSmall,
    ...
    private static DateTime ToNzTime(DateTime utcDateTime)
    {
