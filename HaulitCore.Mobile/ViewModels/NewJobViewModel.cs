...
using HaulitCore.Mobile.Features.Jobs.NewJob;
...
public class NewJobViewModel : BaseViewModel, IQueryAttributable
{
    ...
    // Driver Picker on selected session recall 
    ...
    // Powered vehicle selected session recall 
    ...
    // Trailer slot one selected session recall 
    ...
    // Trailer slot two selected session recall 
    ...
    {
        if (_state.IsLoadingExistingJob)
            return;
