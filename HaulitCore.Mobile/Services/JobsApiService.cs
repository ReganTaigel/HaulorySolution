...
public sealed class JobsApiService : ApiServiceBase
{
    ...
    public Task<object> UpdatePickupDetailsAsync(
        Guid jobId,
        UpdatePickupDetailsRequest request,
        CancellationToken cancellationToken = default)
        => PutAsync<object>($"api/jobs/{jobId}/pickup-details", request, cancellationToken);

    public async Task<IReadOnlyList<JobDto>> GetNeedsReviewJobsAsync(CancellationToken cancellationToken = default)
        => await GetAsync<List<JobDto>>("api/jobs/needs-review", cancellationToken) ?? new List<JobDto>();

    public Task<object> ReviewJobAsync(
        Guid jobId,
        ReviewJobRequest request,
        CancellationToken cancellationToken = default)
        => PutAsync<object>($"api/jobs/{jobId}/review", request, cancellationToken);
}