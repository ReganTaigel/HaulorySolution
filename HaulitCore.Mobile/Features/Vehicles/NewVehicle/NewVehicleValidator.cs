...
public sealed class NewVehicleValidator
{
    public bool CanSave(NewVehicleFormState state)
    {
        ...
        if (OdoCount(state) > 0)
        {
            if (!ShowHubodometers(state)) return false;
            if (IsPoweredVehicle(state) && state.PowerUnitHubodometerKm == null) return false;
            if (IsHeavyTrailer(state) && state.Trailer1HubodometerKm == null) return false;
            if (IsBTrain(state) && state.Trailer2HubodometerKm == null) return false;
        }
    ...
    public bool ShowHubodometers(NewVehicleFormState state) =>
    ...
    public bool ShowPowerUnitOdo(NewVehicleFormState state) => ShowHubodometers(state) && IsPoweredVehicle(state);
    public bool ShowTrailer1Odo(NewVehicleFormState state) => ShowHubodometers(state) && IsHeavyTrailer(state);
    public bool ShowTrailer2Odo(NewVehicleFormState state) => ShowHubodometers(state) && IsBTrain(state);
    ...
    public bool ShowRucStep(NewVehicleFormState state) => ShowHubodometers(state) && RucCount(state) > 0;
