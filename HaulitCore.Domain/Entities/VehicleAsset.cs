...
public class VehicleAsset
{
    ...
    public int? RucHubodometerAtPurchaseKm { get; set; }
    ...
    // Convenience: at what Hubodometer the next RUC top-up is due
    // (usually RucHubodometerAtPurchaseKm + RucDistancePurchasedKm)
    public int? RucNextDueHubodometerKm { get; set; }

    // RUC licence range (the current/last licence bought)
    public int? RucLicenceStartKm { get; set; }   // start Hubodometer on licence (e.g. 1000)
    public int? RucLicenceEndKm { get; set; }     // end Hubodometer on licence (e.g. 2000)
    ...
    public string HubodometerFullDisplay => HubodometerKm == null ? "—" : $"{HubodometerKm.Value:N0} km";
    ...
    public int? RucKmRemaining =>
        (IsRucApplicable && HubodometerKm != null && RucLicenceEndKm != null)
            ? Math.Max(0, RucLicenceEndKm.Value - HubodometerKm.Value)
            : null;
    ...
    public bool IsRucOverdue =>
        IsRucApplicable && HubodometerKm != null && RucLicenceEndKm != null
        && HubodometerKm.Value > RucLicenceEndKm.Value;
    ...
    public void UpdateRucPurchase(int odometerAtPurchaseKm, int distancePurchasedKm, DateTime purchasedDateUtc)
    {
        ...
        RucHubodometerAtPurchaseKm = odometerAtPurchaseKm;
        RucDistancePurchasedKm = distancePurchasedKm;
        RucPurchasedDate = purchasedDateUtc;
        RucNextDueHubodometerKm = odometerAtPurchaseKm + distancePurchasedKm;
    }
